...
void validate_classifier_single(char *datacfg, char *filename, char *weightfile)
{
...
#ifdef PRUNE
    for (int idx = 0; idx < PRUNE_TEST_LEN; idx++) {
        dynamic_epsilon = test_epsilon[idx];
        prune_init_valid();
#endif
    ...
    float end_time = sec(clock() - start_t);
#ifndef SAT_FEATURE
    printf("Zeep Avg Predict time: %f \n", end_time / m);
#endif

#ifdef PRUNE
        prune_output_valid(m, dynamic_epsilon, end_time / m, avg_acc / m, avg_topk / m);
    }
#endif
}

void mvalidate_classifier_single(char *datacfg, char *filename, char *weightfile)
{
    network *net = load_network(filename, weightfile, 0);
    set_batch_network(net, 1);
    srand(222222);
    ...
    for(int i = 0; i < m; ++i){
        ...
        image im = m_load_image_color(paths[i], 0, 0);
...
void mpredict_classifier(char *datacfg, char *cfgfile, char *weightfile, char *filename, int top)
{
    network *net = load_network(cfgfile, weightfile, 0);
    set_batch_network(net, 1);
    srand(2222222);
    ...
    while(1){
        ...
        image im = m_load_image_color(input, 0, 0);
        //For MobileNet v2
...
void run_classifier(int argc, char **argv)
{
    ...
    else if(0==strcmp(argv[2], "mpredict")) mpredict_classifier(data, cfg, weights, filename, top);
    else if(0==strcmp(argv[2], "mvalid")) mvalidate_classifier_single(data, cfg, weights);
}
